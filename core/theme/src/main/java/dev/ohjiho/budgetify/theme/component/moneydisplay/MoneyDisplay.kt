...
class MoneyDisplay @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0, defStyleRes: Int = 0) :
    ...
    var currency: Currency = Currency.getInstance(getLocale(context))
        set(value) {
            field = value
            decimalDigits = currency.getDecimalAmount()
            binding.currency.text = currency.currencyCode
            binding.amount.text = amount.toCurrencyFormat(currency, context)
        }
    private var decimalDigits = 2
        private set(value) {
            ...
            if (value == 0) {
                decimalAmount = ""
            } else if (decimalAmount.length > value) {
                decimalAmount.dropLast(decimalAmount.length - value)
            }
        }
    ...
    fun getAmount() = BigDecimal("$amount.$decimalAmount")

    fun addDigit(digit: Int) {
        if (!inDecimalMode) {
            if (amount.isEmpty() && digit == 0) return
            amount += digit
        } else if (decimalAmount.length < decimalDigits) {
            decimalAmount += digit
        }
    }
